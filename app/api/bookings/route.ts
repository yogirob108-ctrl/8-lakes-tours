...
export async function POST(request: Request) {
  ...
  const normalized = normalizePublicBookingPayload(payload) as
    ...
    | { ok: true; value: {
      ...
      travellers: Array<{
        ...
        email: string | null; phone: string | null; nationality: string;
  ...
  const travellerNames = travellers
    .map((traveller: { position: number; first_name: string; last_name: string; is_lead: boolean }) => `${traveller.position}. ${traveller.first_name} ${traveller.last_name}${traveller.is_lead ? ' (lead)' : ''}`)
    .join('\n');
  const bookingNotes = [
