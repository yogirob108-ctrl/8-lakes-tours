...
export function recoveryEmail(booking, url, stage='abandoned_checkout_1') {
 ...
 const availability='Your place is not confirmed. Places remain subject to availability.';
 ...
 for(const booking of rows || []) {
  ...
  try {
   for(const id of evidence.session_ids) {
    ...
    if(session.status==='expired'){expiredSessions.push(id);continue;}
  ...
  // Only after every owned provider session has passed the paid/unknown/
  // processing fence may exact expired evidence release its matching payment
  // allocation. `false` is the safe default-off/no-match result; an RPC error
  // is unknown and must not advance to a reminder claim.
  if (!dryRun) {
   for (const sessionId of expiredSessions) {
    const {error:releaseError}=await db.rpc('release_departure_capacity_if_safe',{p_booking_id:booking.booking_id,p_session_id:sessionId,p_provider_terminal:'expired'});
    if (releaseError) {suppress('evidence_unavailable');verified=false;break;}
   }
