...
// Subtle plain-text style section rule for the longer customer recovery email.
const DASH_RULE_TEXT = '--------------------------------';
const sectionRuleHtml = () => `<div style="border-top:1px dashed #cccccc;margin:0 0 16px"></div>`;
...
// 24h after stage 1 actually completed. Stage state lives in durable SQL.
...
// - stage 2 due 24h after stage-1 DURABLE COMPLETION however late that was
...
export async function runAbandonedCheckoutRecovery({db,allowedDates,recoveryUrl,sendEmail,retrieveSession,dryRun=false,now=()=>new Date()}) {
 ...
 for(const booking of rows || []) {
  ...
  const {data:claim,error:claimError}=await db.rpc('claim_abandoned_checkout',{p_booking_id:booking.booking_id,p_allowed_dates:allowedDates,p_payload:{...recoveryEmail(booking,recoveryUrl(booking.public_reference)),stage}});
