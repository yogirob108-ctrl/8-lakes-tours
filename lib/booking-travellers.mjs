import { isValidBookingEmail, normalizeBookingEmail } from './email-validation.mjs';
...
export function normalizeBookingTravellers(guestCountValue, travellerValues, options = {}) {
  ...
  for (let index = 0; index < travellerValues.length; index += 1) {
    ...
    const email = normalizeBookingEmail(raw.email);
    ...
    if (!firstName || !lastName || !nationality || !gender || !dateOfBirth || !ridingExperience) {
      return { ok: false, error: `Traveller ${index + 1} requires passport/legal first and last names, date of birth, nationality, gender, and riding level.` };
    }
    for (const [field, value] of Object.entries({ first_name: firstName, last_name: lastName, email, phone, nationality, gender, dietary_notes: dietaryNotes })) {
    ...
    if (!GENDERS.includes(gender)) {
      return { ok: false, error: `Traveller ${index + 1} gender is invalid.` };
    }
