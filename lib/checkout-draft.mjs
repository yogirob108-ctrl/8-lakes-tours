...
const dateParts = item => {
  const values = { date_of_birth: text(item?.date_of_birth, 10), date_of_birth_day: text(item?.date_of_birth_day, 2), date_of_birth_month: text(item?.date_of_birth_month, 2), date_of_birth_year: text(item?.date_of_birth_year, 4) };
...
export function sanitizeDraftPayload(input = {}) {
  ...
  const travellers = Array.isArray(raw.travellers) ? raw.travellers.slice(0, 8).map(item => ({
    first_name: text(item?.first_name, 100), last_name: text(item?.last_name, 100), ...dateParts(item),
  })) : [];
  return {
    first_name: text(raw.first_name, 100), last_name: text(raw.last_name, 100), email: email(raw.email),
    phone: text(raw.phone, 40), ...dateParts(raw), tour_date: text(raw.tour_date, 160),
