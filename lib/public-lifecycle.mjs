...
export function reconcileStripeProviderEvidence({ bookings, evidence, approvedBindings, scanComplete, scanIncompleteReason, scanIncompleteCollection }) {
  return bookings.map((booking) => {
    ...
    if (bound) {
      ...
      // carry the reference must not create a false conflict. Deduplicate the
      // full evidence set so one PaymentIntent observed through several
      // collection views is evaluated exactly once.
      const global = new Map();
      for (const item of evidence) {
        const key = evidenceKey(item);
        const prior = global.get(key);
        if (!prior || item.id === bound.provider_object_id) global.set(key, item);
      }
      const boundMatches = [...global.values()].filter(item => item.id === bound.provider_object_id
