...
export function reconcileStripeProviderEvidence({ bookings, evidence, approvedBindings, scanComplete, scanIncompleteReason, scanIncompleteCollection }) {
  return bookings.map((booking) => {
    ...
    if (bound) {
      ...
      // Evaluate exactly one representative per provider payment, merging
      // complementary observations — keep the live-status row and fill its
      // gaps from siblings. The bound object's own customer email (the chain
      // the verified bind action checked) wins for the merged representative.
      ...
      for (const item of boundObservations) {
        ...
        if (!prior) { representatives.set(key, { ...item }); continue; }
        for (const [field, value] of Object.entries(item)) {
          if (prior[field] === undefined || prior[field] === null || prior[field] === '') prior[field] = value;
        }
        if (item.id === bound.provider_object_id && item.customer_email) prior.customer_email = item.customer_email;
      }
