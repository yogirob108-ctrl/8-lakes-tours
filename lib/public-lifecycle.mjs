 ...
 */
export function reconcileStripeProviderEvidence({ bookings, evidence, scanComplete }) {
  return bookings.map((booking) => {
    ...
    // A complete session alone cannot rule out an undiscovered refund or a
    // conflicting provider object when any required collection was truncated.
    if (!scanComplete) return { reference: booking.reference, status: 'scan_incomplete_unknown' };

    const byProviderPayment = new Map();
    for (const item of referenceMatches) {
      const key = evidenceKey(item);
      const prior = byProviderPayment.get(key);
      // A Checkout Session is the richer representation of its own PaymentIntent.
      if (!prior || item.source === 'checkout_session') byProviderPayment.set(key, item);
    }
    const matches = [...byProviderPayment.values()];
    if (matches.length !== 1) return { reference: booking.reference, status: 'review_multiple_conflicting_matches' };
