...
export async function collectStripeLifecycleEvidence({ stripe, pageBudget = 90 }) {
  ...
  const collections = [
    ...
    { list: stripe.invoices.list.bind(stripe.invoices), source: 'invoice', options: { expand: ['data.payment_intent.latest_charge', 'data.payments.data.payment.payment_intent.latest_charge'] } },
  ];
