  ...
  for (const item of incomplete) {
    ...
    try {
      ...
      if (item.payment_intent_id && !item.payment_intent_status) {
        ...
        // The invoice LIST row can omit customer_email that the retrieve
        // (verified bind chain) carries; complete it from the same source.
        if (!item.customer_email) {
          const full = await stripe.invoices.retrieve(item.id, { expand: ['payments.data.payment.payment_intent'] });
          item.customer_email = item.customer_email || string(full?.customer_email);
        }
      }
    } catch (error) {
