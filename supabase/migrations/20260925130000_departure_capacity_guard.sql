...
begin
 ...
 if not found or b.departure_id is null or p_expected is null or not(to_jsonb(b)@>p_expected) or b.status not in ('awaiting_payment','application_received') or b.online_paid_usd<>0 then raise exception 'booking changed or not payable; operator review required'; end if;
 select * into d from public.departures where id=b.departure_id and published and capacity_enforced for update;
 if not found then raise exception 'departure capacity is not activated or date is unknown; operator review required'; end if;
 select coalesce(sum(guest_count),0) into used from public.departure_capacity_allocations where departure_id=d.id and booking_id<>b.id;
...
begin
 ...
 if p_session_id is null or p_url is null or not public.reserve_departure_capacity(b.id,p_session_id) then return false; end if;
...
revoke all on function public.assign_new_booking_departure(),public.reserve_departure_capacity(uuid,text),public.fence_departure_capacity_ops(),public.release_departure_capacity_if_safe(uuid,text) from public,anon,authenticated;
grant execute on function public.reserve_departure_capacity(uuid,text),public.release_departure_capacity_if_safe(uuid,text) to service_role;
