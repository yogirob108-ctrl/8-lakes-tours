...
test('B2: legacy dedupe max2 — journal carries legacy stage-1 completion, runner can only claim stage 2',async()=>{
 ...
 // completion derived from the legacy sent ledger 25h ago (verified timestamp,
 ...
 // stage-2 window.
 const seededStages={abandoned_checkout_1:{completed_at:new Date(NOW.getTime()-25*HOUR).toISOString()}};
 assert.equal(dueStage(seededStages,NOW,{}),'abandoned_checkout_2');
 assert.equal(dueStage(seededStages,new Date(NOW.getTime()-23*HOUR),{}),null);
 // Outside the stage-2 window (legacy send older than 72h): permanently done.
 assert.equal(dueStage({abandoned_checkout_1:{completed_at:new Date(NOW.getTime()-80*HOUR).toISOString()}},NOW,{}),null);
 const sent=[],calls=[];
