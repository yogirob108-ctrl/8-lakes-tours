...
test('recovery sends private transaction only after claim and fresh authorization',async()=>{const h=harness();assert.deepEqual(await h.run(),{sent:1,failed:0,suppressed:0});assert.equal(h.sent[0].idempotencyKey,'public-booking-b-abandoned_checkout');assert.match(h.sent[0].text,/not confirmed/);assert.doesNotMatch(h.sent[0].text,/newsletter|passport|date of birth/i);});
...
test('email escapes private URL and contains no marketing or traveller data',()=>{const e=recoveryEmail(row,'https://example.invalid/pay?a=1&token=<secret>');assert.match(e.html,/&amp;token=&lt;secret&gt;/);assert.match(e.text,/Rob & the 8 Lakes Tours team/);});
