...
test('recovery sends private transaction only after claim and fresh authorization',async()=>{const h=harness();assert.deepEqual(await h.run(),{sent:1,failed:0,suppressed:0,suppressed_reasons:{},stages:{}});assert.equal(h.sent[0].idempotencyKey,'public-booking-b-abandoned_checkout_1');assert.match(h.sent[0].text,/private checkout link/);assert.doesNotMatch(h.sent[0].text,/newsletter|passport|date of birth/i);});
