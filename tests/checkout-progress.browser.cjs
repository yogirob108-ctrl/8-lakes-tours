 ...
 try {
  ...
  for(const scenario of [...Array.from({length:8},(_,i)=>i+1),'failed-checkout','failed-intake','private']) {
   ...
   for(let i=0;i<n;i++){
    ...
    for(const [key,value] of Object.entries({first_name:'Local',last_name:'Fixture',nationality:'Testland'}))await page.locator(`[name="${prefix}${key}"]`).fill(value);
    await page.locator(`[name="${prefix}gender"]`).selectOption('Female');
