...
for (const edit of [{guest_count:4},{guest_count:2},{tour_date:'Other'},{online_due_usd:4000},{online_due_usd:2000}]) test('frozen paid issuance survives commercial edit '+JSON.stringify(edit),async()=>{
 const booking={id:'b',customer_id:'c',public_reference:'REF',status:'awaiting_payment',tour_date:'Scheduled',guest_count:3,online_due_usd:2922,online_paid_usd:0,...edit,customer:{email:'fixture@example.invalid'}};
 ...
 const source=(process.env.REVIEW_RED==='1'?execFileSync('git',['show','eef43718c77231269ea56db70da1a01812eaf745:'+file],{encoding:'utf8'}):readFileSync(new URL('../'+file,import.meta.url),'utf8'))+'\nexport {handleCheckoutSessionPaid};';
 const exports={};
